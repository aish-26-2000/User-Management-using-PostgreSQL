...
exports.authRoute = async (req, res, next) => {
    ...
    if (authheader && authheader.startsWith('Bearer')) {
        ...
        if (!token) {
            responseHelper.fail(res, 'You are not logged in! Please log in to get access');
        }
        const decoded = decode(token);
        const currentUser = await userService.getCurrentUser(decoded);
        if (!currentUser) {
            responseHelper.fail(res, 'The user belonging to this token does no longer exist.');
        }
        if (currentUser) {
            const date = new Date();
            if (decoded.exp < date.getTime() / 1000) {
                responseHelper.fail(res, 'Token expired! Please log in again to get access');
            }
            if (decoded.exp > date.getTime() / 1000) {
                next();
            }
